...
subroutine read_control(fn)
    ...
    do 
        ...
        if (key=='use_aabbtree') read(val, *) use_aabbtree
        ...
        if (key=='lanc_mxitr') read(val,*) lanc_mxitr
        if (key=='lanc_tol') read(val,*) lanc_tol
        if (key=='lanc_tol') read(val,*) lanc_tol
        if (key=='bdintg') read(val,*) bdintg

        if (key=='tim_stp')  tim_stp    = str_to_d(val)
        if (key=='nts_md')   nts_md     = int(str_to_d(val))
        if (key=='nts_sim')  nts_sim  = int(str_to_d(val), ip_long)
        if (key=='nts_mobsam') nts_mobsam = int(str_to_d(val))
...
subroutine write_control(fn)
    ...
    write(fu, '(a,t20,l1)'  ) 'use_aabbtree', use_aabbtree
    ...
    write(fu, '(a,t20,i0)'  ) 'lanc_mxitr', lanc_mxitr
    write(fu, '(a,t20,g0.6)') 'lanc_tol', lanc_tol
    write(fu, '(a,t20,a)'   ) 'bdintg', bdintg
    ...
    write(fu, '(a,t20,i0)')   'nts_mobsam', nts_mobsam
